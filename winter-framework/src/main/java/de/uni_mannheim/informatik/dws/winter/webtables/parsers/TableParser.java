...
public abstract class TableParser {
    ...
    /**
     * Holds typeDetector for data type detection. It is possible to implement your own TypeDetector by using the corresponding Interface.
     */
    private TypeDetector typeDetector;

    /**
     * Holds tableHeaderDetector to Detect a table header. It is possible to implement your own header detector by using the corresponding Interface.
     */
    private TableHeaderDetector tableHeaderDetector;

    /**
     * Holds the stringNormalizer used to normalizes input strings. It is possible to implement your own string normalizer by using the corresponding Interface.
     */
    private StringNormalizer stringNormalizer;

    /**
     * Holds the rowContentDetector used to analyze the table rows before populating the table. It is possible to implement your own row content detector by using the corresponding Interface.
     */
    private RowContentDetector rowContentDetector;

    private boolean checkForList = true;

    public boolean isCheckForList() {
        return checkForList;
    }

    public void setCheckForList(boolean checkForList) {
        this.checkForList = checkForList;
    }
     ...
     *
     * @param tContent has the content, which has to be populated
     * @param t has the table, which is populated
     * @param skipRows holds the indexes for header, empty and sum rows. These rows are skipped, because they do not belong to the content. Likewise these rows do not increase the row id, thus table
     * row id and parsed table row id maid slightly differ depending on the header.
     */
            ...
            if (!ArrayUtils.contains(skipRows, rowIdx)) {
                ...
                for (int i = 0; i < rowData.length && i < values.length; i++) {
                    if (rowData[i] != null && !rowData[i].trim().isEmpty()) {
                        ...
                        if (ListHandler.checkIfList(rowData[i]) && checkForList) {
