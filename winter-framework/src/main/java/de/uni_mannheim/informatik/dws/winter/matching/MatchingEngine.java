...
public class MatchingEngine<RecordType extends Matchable, SchemaElementType extends Matchable> {
    ...
    /**
     * Runs the Duplicate Detection on a given {@link DataSet}. In order to reduce the number of internally compared {@link AbstractRecord}s the functions can be executed in a <i>symmetric</i>-mode.
     * Here it will be assumed, that that the {@link MatchingRule} is symmetric, meaning that score(a,b) = score(b,a). Therefore the pair (b,a) can be omitted. Normally, this option can be set to
     * <b>true</b> in most of the cases, as most of the common similarity functions (e.g. {@link LevenshteinSimilarity}, and {@link TokenizingJaccardSimilarity}) are symmetric, meaning sim(a,b) =
     * sim(b,a).
     *
     * @param dataset The data set
     * @param rule The {@link MatchingRule} that is used to compare the records.
     * @param blocker The blocker to use
    ...
    /**
     * Runs the Duplicate Detection on a given {@link DataSet} with additional schema correspondences. In order to reduce the number of internally compared {@link AbstractRecord}s the functions can be
     * executed in a
     * <i>symmetric</i>-mode. Here it will be assumed, that that the
     * {@link MatchingRule} is symmetric, meaning that score(a,b) = score(b,a). Therefore the pair (b,a) can be omitted. Normally, this option can be set to <b>true</b> in most of the cases, as most
     * of the common similarity functions (e.g. {@link LevenshteinSimilarity}, and {@link TokenizingJaccardSimilarity}) are symmetric, meaning sim(a,b) = sim(b,a).
     *
     * @param dataset The data set
    ...
    public Processable<Correspondence<RecordType, SchemaElementType>> runDuplicateDetection(
        ...
        RuleBasedDuplicateDetectionAlgorithm<RecordType, SchemaElementType> algorithm = new RuleBasedDuplicateDetectionAlgorithm<>(
            dataset, Correspondence.toMatchable(schemaCorrespondences), rule, blocker);
        algorithm.setTaskName("Duplicate Detection");
     ...
     * @param schemaCorrespondences (Optional) Schema correspondences between the data sets that tell the matching rule which attribute combinations to compare.
     * @param rule The {@link MatchingRule} that is used to compare the records.
     * @param blocker The {@link AbstractBlocker} that generates the pairs, which are then checked by the {@link MatchingRule}.
     * @return A list of correspondences
     */
    public Processable<Correspondence<RecordType, SchemaElementType>> runIdentityResolution(
        ...
        RuleBasedMatchingAlgorithm<RecordType, SchemaElementType, SchemaElementType> algorithm = new RuleBasedMatchingAlgorithm<>(
            dataset1, dataset2, Correspondence.toMatchable(schemaCorrespondences), rule, blocker);
    ...
    /**
     * Runs blocking on the given data sets using the provided blocker. Using a dummy matching rule all blocked pairs are matched.
     *
     * @param dataset1 The first data set
     * @param dataset2 The second data set
     * @param schemaCorrespondences (Optional) Schema correspondences between the data sets that tell the matching rule which attribute combinations to compare.
     * @param blocker The {@link AbstractBlocker} that generates the pairs, which are then matched by the dummy {@link MatchingRule}.
     * @return A list of correspondences
     */
     ...
     * @param blockingfunction1 the blocking function for the first dataset
     * @param blockingfunction2 the blocking function for the second dataset
     * @param vectorCreation the vector creation method
     * @param similarity the similarity function
     * @param similarityThreshold the similarity threshold
     * @return the resulting correspondences
     */
    public Processable<Correspondence<RecordType, MatchableValue>> runVectorBasedIdentityResolution(
        ...
        VectorCreationMethod vectorCreation, VectorSpaceSimilarity similarity, double similarityThreshold) {
    ...
    /**
     * Runs schema matchign on the given data sets using the provided matching rule and blocker. The data sets must contain the attribute features as records.
     *
     * @param schema1 The first data set
     * @param schema2 The second data set
     * @param instanceCorrespondences (Optional) Instance correspondences between the data sets that tell the matching rule which instance values to compare.
     * @param rule The {@link MatchingRule} that is used to compare the records.
     * @param blocker The {@link AbstractBlocker} that generates the pairs, which are then checked by the {@link MatchingRule}.
     * @return A list of correspondences
     */
    public Processable<Correspondence<SchemaElementType, RecordType>> runSchemaMatching(
        ...
        RuleBasedMatchingAlgorithm<SchemaElementType, SchemaElementType, RecordType> algorithm = new RuleBasedMatchingAlgorithm<>(
            schema1, schema2, Correspondence.toMatchable(instanceCorrespondences), rule, blocker);
     ...
     * @param labelComparator The comparator that compares the labels of two attributes
     * @param similarityThreshold The similarity threshold for creating correspondences
     * @return The discovered schema correspondences
     * @throws Exception From LinearCombinationMatchingRule
     */
    public Processable<Correspondence<SchemaElementType, SchemaElementType>> runLabelBasedSchemaMatching(
        DataSet<SchemaElementType, SchemaElementType> schema1,
        DataSet<SchemaElementType, SchemaElementType> schema2,
        Comparator<SchemaElementType, SchemaElementType> labelComparator, double similarityThreshold)
        throws Exception {
    ...
    /**
     * Runs duplicate-based schema matching between the provided schemas. First, the {@link VotingMatchingRule} rule is evaluated for all provided instance correspondences to create votes. Optionally,
     * these votes can be filtered by the voteFilter, i.e. to limit the number of votes that each value can cast. Then, the {@link CorrespondenceAggregator} voteAggregator combines the votes into
     * final correspondences.
     *
     * @param schema1 The first schema
     * @param schema2 The second schema
     * @param instanceCorrespondences The instance correspondences (=duplicates)
     * @param rule The rule to cast votes from instance correspondences
     * @param voteFilter The filtering rule for votes (optional)
     * @param voteAggregator The aggregation from votes to final correspondences
     * @param schemaBlocker The blocker that creates potential pairs of attributes. Use {@link NoSchemaBlocker} is all combinations should be considered.
    ...
    public Processable<Correspondence<SchemaElementType, RecordType>> runDuplicateBasedSchemaMatching(
        ...
        DuplicateBasedMatchingAlgorithm<RecordType, SchemaElementType> algorithm = new DuplicateBasedMatchingAlgorithm<>(
            schema1, schema2, Correspondence.toMatchable(instanceCorrespondences), rule, voteFilter, voteAggregator,
            schemaBlocker);
     ...
     *
     * @param correspondences The correspondences that should be filtered
     * @param k The desired number of correspondences for each record on the left-hand side
     * @param similarityThreshold The minimum similarity for a correspondence to appear in the result
    ...
    public Processable<Correspondence<RecordType, SchemaElementType>> getTopKInstanceCorrespondences(
        Processable<Correspondence<RecordType, SchemaElementType>> correspondences, int k,
        double similarityThreshold) {
        TopKCorrespondencesAggregator<RecordType, SchemaElementType> aggregator = new TopKCorrespondencesAggregator<>(
            k);

        return correspondences.aggregate(new AggregateByFirstRecordRule<>(similarityThreshold), aggregator)
     ...
     *
     * @param correspondences The correspondences that should be filtered
     * @param k The desired number of correspondences for each attribute on the left-hand side
     * @param similarityThreshold The minimum similarity for a correspondence to appear in the result
     * @return The top k correspondences
     */
    public Processable<Correspondence<SchemaElementType, RecordType>> getTopKSchemaCorrespondences(
        Processable<Correspondence<SchemaElementType, RecordType>> correspondences, int k,
        double similarityThreshold) {
        TopKCorrespondencesAggregator<SchemaElementType, RecordType> aggregator = new TopKCorrespondencesAggregator<>(
            k);
    ...
    public Processable<Correspondence<RecordType, SchemaElementType>> getMaximumWeightGlobalInstanceMatching(
        ...
        MaximumBipartiteMatchingAlgorithm<RecordType, SchemaElementType> maximumMatching = new MaximumBipartiteMatchingAlgorithm<>(
            correspondences);
    ...
    public Processable<Correspondence<SchemaElementType, RecordType>> getMaximumWeightGlobalSchemaMatching(
        ...
        MaximumBipartiteMatchingAlgorithm<SchemaElementType, RecordType> maximumMatching = new MaximumBipartiteMatchingAlgorithm<>(
            correspondences);
